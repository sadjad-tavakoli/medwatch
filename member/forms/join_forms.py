...
class DrJoinForm(forms.Form):
    ...
    firstName = forms.CharField(max_length=100, required=True,
                                widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'نام'}))
    lastName = forms.CharField(max_length=100, required=True,
                               widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'نام خانوادگی'}))
    national_id = forms.IntegerField(max_value=9999999999, required=True,
                                     widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'کد ملی'}))
    # birth_date = forms.DateInput(required=False)

    username = forms.CharField(max_length=100, required=True,
                               widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'نام کاربری'}))
    password = forms.CharField(widget=forms.PasswordInput(attrs={'placeholder': 'رمز عبور', 'class': 'input-field'}),
                               required=True)
    passwordRepeat = forms.CharField(
        widget=forms.PasswordInput(attrs={'placeholder': 'تکرار رمز عبور', 'class': 'input-field'}), required=True)
    email = forms.EmailField(widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'پست الکترونیک'}))
    degree = forms.CharField(max_length=100, required=True,
                             widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'مدرک تحصیلی'}))
    university = forms.CharField(max_length=100, required=True,
                                 widget=forms.TextInput(
                                     attrs={'class': 'input-field', 'placeholder': 'دانشگاه فارغ التحصیلی'}))
    graduate_year = forms.IntegerField(required=True,
                                       widget=forms.TextInput(
                                           attrs={'class': 'input-field', 'placeholder': 'سال اخذ مدرک'}))
    ...
    def clean(self):
        ...
        #     doctor = DoctorMember.objects.create(username=data['username'],
